...
class PackageManager:
    def __init__(self):
        ...
        if not os.path.exists(self._file_name):
            ...
            logging.debug(
                "PackageManager: located system python packages: "
                + str(len(self._packages))
            )
        ...
        for p in packs:
            ...
            pkg = next(
                (
                    ...
                    if cast(PythonPackage, pkg).name == p.name
                ),
                None,
            )
            if pkg is not None:
            ...
            result = subprocess.run(
                ["pip", "install", package_str], capture_output=True, text=True
            )
    ...
    def delete(self, packs: list[PythonPackage]) -> None:
        ...
        for p in packs:
            ...
            if cast(PythonPackage, pkg).system is True:
                logging.warning(
                    f"PackageManager: Package {p.name} is system. I am unable to uninstall it."
                )
                continue

            package_str = (
                f"{cast(PythonPackage, pkg).name}=={cast(PythonPackage, pkg).version}"
            )
            result = subprocess.run(
                ["pip", "uninstall", "-y", package_str], capture_output=True, text=True
            )
            logging.debug("PackageManager: " + result.stdout)
            logging.error("PackageManager: " + result.stderr)
            if result.returncode != 0:
    ...
    def _get_all_packages(self, system: bool = False) -> list[PythonPackage]:
        ...
        for line in packages:
            row = line.split("==")
            out.append(PythonPackage(name=row[0], version=row[1], system=system))
        return out
