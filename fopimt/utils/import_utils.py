...
def dynamic_import(module_name, specific_part=None, alias=None, exec_globals=None):
    ...
    try:
        if specific_part:
            ...
            exec_globals[alias if alias else specific_part] = getattr(
                module, specific_part
            )
            logging.debug(
                f"Utils:Import: Imported {specific_part} from {module_name} as {alias if alias else specific_part}"
            )
        else:
            # Import the whole module
            exec_globals[alias if alias else module_name] = __import__(module_name)
            logging.debug(
                f"Utils:Import: Imported {module_name} as {alias if alias else module_name}"
            )
    except ImportError:
    ...
    try:
        ...
        if not pip_package:
            ...
            if pip_package:
                ...
                logging.debug(
                    f"Utils:Install: Found {module_name} on PyPI as {pip_package}. Adding to mappings."
                )
            else:
                # Fallback: assume module_name is the pip package name
                logging.debug(
                    f"Utils:Install: {module_name} module not found in module mapping or PyPI. Is it spelled correctly?"
                )
                raise ImportError(
                    f"{module_name} module not found in module mapping or PyPI. Is it spelled correctly?"
                )
        ...
        try:
            ...
            logging.error(
                f"Utils:Install: {module_name} module cannot be installed. Is it spelled correctly? {repr(e)}"
            )
            raise e
