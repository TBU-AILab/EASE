...
def _plot_points_in_triangle(fitness: float, points: np.ndarray, filepath: str):
    ...
    triangle_patch = patches.Polygon(
        triangle_vertices, closed=True, edgecolor="black", facecolor="none"
    )
    ...
    ax.set_ylim(-0.1, 1.0)
    ax.set_title(
        f"Heilbronn triangles solution for n={len(points[:, 0])}, fitness={fitness}"
    )
    ax.set_axis_off()
...
class StatHeilbronn(Stat):
                ...
                if solution.get_metadata().get("results"):
                    if solution.get_metadata().get("results").get("fitness"):
                        if (
                            ...
                            self._data = {
                                "coordinates": solution.get_metadata()
                                .get("results")
                                .get("coordinates"),
                                "fitness": solution.get_metadata()
                                .get("results")
                                .get("fitness"),
                            }
                            best_fitness = (
    ...
    def export(self, path: str):
        if self._data:
            _plot_points_in_triangle(
                self._data["fitness"],
                self._data["coordinates"],
                os.path.join(path, "stat_" + self.get_short_name() + ".png"),
            )
    ...
    def get_tags(cls) -> dict:
        return {"input": {"heilbronn"}, "output": set()}
