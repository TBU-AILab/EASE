...
class StatMetaheuristic(Stat):
    ...
    def get_description(cls) -> str:
        return (
            "Statistical module for metaheuristic algorithms. Compute descriptive statistics and pairwise "
            "comparison and ranking."
        )

    @classmethod
    def get_tags(cls) -> dict:
        return {"input": {"metaheuristic"}, "output": set()}
        ...
        for dim in dims:
            ...
            for fun in functions:
                ...
                for row_player in algorithms:
                    ...
                    new_row = {
                        "algorithm": row_player,
                        "function": fun,
                        "dim": dim,
                        "min": min(rp_result),
                        "max": max(rp_result),
                        "median": np.median(rp_result),
                        "mean": np.mean(rp_result),
                    ...
                    for col_player in algorithms:
                        if row_player == col_player:
                            result.loc[row_player, col_player] = (
                                "0"  # No match against themselves
                            )
                        else:
                            # Result
                            rp_result = list(
                            ...
                            result.loc[row_player, col_player] = (
                                sign  # Random result for illustration
                            )

                result["ranking"] = result["score"].rank(
                ...
                for index, _ in result.iterrows():
                    desc_stats.loc[
                        (desc_stats["algorithm"] == index)
                        & (desc_stats["function"] == fun)
                        & (desc_stats["dim"] == dim),
                        "ranking",
                    ] = result.loc[index, "ranking"]

                results.append({"dim": dim, "func": fun, "result": result})

            for a in algorithms:
                df_rank_dim.loc[a, "score"] = dict_rank_dim[a]

            df_rank_dim["ranking"] = df_rank_dim["score"].rank(
                method="average", ascending=False
            )
            df_rank_dim = df_rank_dim.sort_values(by="ranking")

            rank_dims.append({"dim": dim, "df_rank_dim": df_rank_dim})

        for a in algorithms:
            df_rank_all.loc[a, "score"] = dict_rank_all[a]

        df_rank_all["ranking"] = df_rank_all["score"].rank(
            method="average", ascending=False
        )
