...
class Magic:
    ...
    def task_pause(self, task_id: str) -> bool:
        if task_id == None or task_id not in self._tasks.keys():
            logging.error(
                f"Magic:Task Pause: Task with id({task_id}) does not exist in the system."
            )
            return False

        if self._tasks[task_id].state != TaskState.RUN:
            logging.error(
                f"Magic:Task Pause: Task with id({task_id}) is not in the RUN state."
            )
            return False
    ...
    def task_stop(self, task_id: str) -> bool:
        if task_id is None or task_id not in self._tasks.keys():
            logging.error(
                f"Magic:Task Stop: Task with id({task_id}) does not exist in the system."
            )
            return False

        if self._tasks[task_id].state not in [TaskState.RUN, TaskState.PAUSED]:
            logging.error(
                f"Magic:Task Stop: Task with id({task_id}) is not in the RUN state."
            )
            return False
    ...
    def task_run(self, uid: str) -> bool:
        ...
        if uid in self._tasks.keys():
            ...
            if task.state != TaskState.INIT and task.state != TaskState.PAUSED:
                logging.info(
                    f"Magic:Task Run: Task with id({uid}) is not fully initialized."
                )
                return False
        else:
            logging.info(
                f"Magic:Task Run: Task with id({uid}) does not exist in the system."
            )
            return False
        ...
        try:
            ...
            logging.error(
                f"Magic:Task Run: Task with id({uid}) could not be run due to: repr{e}"
            )
    ...
    def task_duplicate(self, orig_task: Task, new_name: str) -> Task:
        if not hasattr(orig_task, "_init_config"):
            logging.error(
                f"Error while duplicating: Task with ID:{orig_task.id} does not have a valid _init_config attribute."
            )
            return None
        ...
        if orig_task._init_config is None:
            logging.warning(
                f"Task with ID:{orig_task.id} does not have a valid _init_config attribute. Duplicating as a clean task."
            )
        ...
        logging.info(
            f"Duplicating task with ID:{orig_task.id} to new task with name:{new_name}."
        )
        new_task = self.task_create()
    ...
    def _init(self) -> None:
        ...
        task_folders = [
            item
            for item in os.listdir(self._dir_tasks)
            if os.path.isdir(os.path.join(self._dir_tasks, item))
            and item.startswith("t_")
        ]
        ...
        for folder in task_folders:
            try:
                ...
                logging.error(
                    f"Unable to create instance of Task from storage:{folder}. {e}"
                )
