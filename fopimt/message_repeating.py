...
class MessageRepeating(Message):
    def __init__(
        ...
        if isinstance(self._msgs, list) and len(self._msgs) == 0:
            ...
            logging.warning(
                "RepeatingMessage: Parameter msgs set incorrectly. No specified message/s. Setting "
                'message to "Improve."'
            )

        if (
            isinstance(self._msgs, str)
            and self._type != MessageRepeatingTypeEnum.SINGLE
        ):
            self._type = MessageRepeatingTypeEnum.SINGLE
            logging.warning(
                "RepeatingMessage: Parameter msgs set incorrectly. Switching to SINGLE mode."
            )
        ...
        if self._type == MessageRepeatingTypeEnum.RANDOM_WEIGHTED and (
            ...
            logging.warning(
                "RepeatingMessage: Parameter weight set incorrectly. Switching to RANDOM mode."
            )
