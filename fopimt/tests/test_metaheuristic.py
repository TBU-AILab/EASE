...
class TestMetaheuristic(Test):
    ...
    def _init_params(self):
        ...
        self._max_evals = (
            997  # Default value maybe changed in the future as optional argument
        )
    ...
    def get_description(cls) -> str:
        return (
            "This class serves for metaheuristic testing. Tests common issues with metaheuristic algorithms:\n"
            "- Maximum evaluation count exceeded\n"
            "- Search space bounds breaches\n"
            "- Evaluation of wrong length individuals\n"
        )
    ...
    def test(self, solution: Solution) -> bool:
        ...
        if result.get("maxevalexception", False):
            self._error_msg = self._user_msg = (
                "Test:Metaheuristic: Algorithm tried to exceed maximum number of "
                "evaluations."
            )
            self._test_result = True
        ...
        if result.get("outofboundsexception", False):
            self._error_msg = self._user_msg = (
                "Test:Metaheuristic: Algorithm is crossing the bounds of the specified "
        ...
        if result.get("dimexception", False):
            self._error_msg = self._user_msg = (
                "Test:Metaheuristic: Algorithm tried to evaluate solution of "
        ...
        if result.get("unexpectedexception", False):
            self._error_msg = self._user_msg = (
                f"Test:Metaheuristic: {result.get('unexpectedexception')}"
            )
            self._test_result = False
