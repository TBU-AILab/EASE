...
class TestHeilbronn(Test):
    ...
    def get_description(cls) -> str:
        return (
            "This class serves for Heilbronn Triangle solver testing. Tests common issues with solutions:\n"
    ...
    def test(self, solution: Solution) -> TestResult:
        ...
        try:
            ...
            coordinates, _ = algorithm(
                11, _evaluate_found_points, _check_inside_triangle, MAX_EVAL_TIME
            )
            end_time = time.time()

            if end_time - start_time > (MAX_EVAL_TIME + 1):
                self._error_msg = self._user_msg = (
                    f"Test:Heilbronn: Generated algorithm violated the time constraint ({MAX_EVAL_TIME} +1 (for orchestration)s), evaluation time was: {end_time - start_time}s"
                )
                self._test_result = False

            if not _check_inside_triangle(coordinates):
                self._error_msg = self._user_msg = (
                    "Test:Heilbronn: Generated algorithm violated the triangle constraint."
                )
                self._test_result = False
