...
""",
    ...
    def evaluate(self, solution: Solution) -> EvaluatorResult:
        ...
        try:
            ...
            (
                ...
                outer_hex_side_length,
                outer_hex_angle_degrees,
            ) = algorithm(self.n, verify_construction, self.time_constraint)
            ...
            self._keys = {
                "best_inner_hex_data": self._best.get_metadata()
                .get("results")
                .get("inner_hex_data"),
                "best_outer_hex_center": self._best.get_metadata()
                .get("results")
                .get("outer_hex_center"),
                "best_outer_hex_side_length": self._best.get_metadata()
                .get("results")
                .get("outer_hex_side_length"),
                "best_outer_hex_angle_degrees": self._best.get_metadata()
                .get("results")
                .get("outer_hex_angle_degrees"),
                "inner_hex_data": solution.get_metadata()
                .get("results")
                .get("inner_hex_data"),
                "outer_hex_center": solution.get_metadata()
                .get("results")
                .get("outer_hex_center"),
                "outer_hex_side_length": solution.get_metadata()
                .get("results")
                .get("outer_hex_side_length"),
                "outer_hex_angle_degrees": solution.get_metadata()
                .get("results")
                .get("outer_hex_angle_degrees"),
    ...
    def get_description(cls) -> str:
        return (
            "Evaluator for algorithms solving Hexagon. Assuming generated Python code with "
            "template code of the runner."
        )

    @classmethod
    def get_tags(cls) -> dict:
        return {"input": {"python"}, "output": {"Hexagon"}}
