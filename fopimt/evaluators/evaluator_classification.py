...
class EvaluatorClassification(Evaluator):
    ...
    def get_parameters(cls) -> dict[str, Parameter]:
        ...
        return {
            ...
            "keywords": Parameter(
                ...
                enum_options=["accuracy", "precision", "recall", "f1-score", "errors"],
                readonly=True,
            ),
            "error_limit": Parameter(
                short_name="error_limit",
                description="Number of reported wrong classifications",
    ...
    def _init_params(self):
        ...
        self._error_limit = self.parameters.get(
            "error_limit", self.get_parameters().get("error_limit").default
        )
        ...
        try:
            ...
            for i in range(len(self._test["X"])):
                if y_true[i] != y_pred[i]:
                    errors.append(
                        {"x": self._test["X"][i], "t": y_true[i], "y": y_pred[i]}
                    )
            ...
            logging.error(
                "Evaluator:Classification: Error during Task evaluation: " + repr(e)
            )
            raise e
    ...
    def get_tags(cls) -> dict:
        return {"input": {"python"}, "output": {"classification"}}
