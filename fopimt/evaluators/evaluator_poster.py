...
class EvaluatorPoster(Evaluator):
...
Notes:
Use a combination of image processing libraries, such as OpenCV, Pillow, scikit-image, TensorFlow, dlib, or face_recognition depending on the genre.
Select only 5 images each time the algorithm runs, and provide a rating for each (in the range 0-1).
The rating should reflect how well the image meets the criteria for a promotional poster, which may differ by genre.
Do not include any example usage or direct method calls. Only the implementation of the function is required.
                                           """,
                                           readonly=True),
    ...
    def get_long_name(cls) -> str:
        return "ModernTV Poster evaluator"

    @classmethod
    def get_description(cls) -> str:
        return "ModernTV evaluator for selected posters."
